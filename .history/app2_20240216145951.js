...
//后面要独立出来 因为these are all 字符串，字符串写死了arleady, but use JS传到字符串里面去
...
for(let i = 0; i < n; i++){
    ...
    let a_position = gl.getAttribLocation(gl.program, 'a_position') //传参：我们WebGL的名称：program
    console.log(location) //获取成功是0，失败是-1

    //写法2
    gl.vertexAttrib2f(a_position, x, y) //ES6写法
    //gl.vertexAttrib2fv(location, new Float32Array(position)) //both ok *这里2fv 有v

    let a_size = gl.getAttribLocation(gl.program, 'a_size') 
    gl.vertexAttrib1f(a_size, r * 5) //半径越大 点越大（越往外）
    //画一个点
